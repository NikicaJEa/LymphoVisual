...
def IGH_FR2(folder_name,loc,loc_save,region,patient_id,region_graph,dic,svr,svc,sjr,sjc,srr,src,
    ...
    if num_stacked_items > 0:
        ...
        with open(folder_name + "/IGH_FR2_VJ_Sequence_Frequencies.tsv", "w") as f:
        ...
        copyfile(os.path.dirname(os.path.abspath(inspect.stack()[0][1])) + "/IGH_FR2_VJ_Sequence_Frequencies.gnuplot",
                 folder_name + "/IGH_FR2_VJ_Sequence_Frequencies.gnuplot")

        os.chdir(folder_name)

        print("Executing gnuplot")
        os.system("gnuplot IGH_FR2_VJ_Sequence_Frequencies.gnuplot")

        try:
            move("./IGH_FR2_VJ_Sequence_Frequencies.png", "./" + patient_id + "_IGH_FR2_VJ_Sequence_Frequencies.png")

            os.remove("./IGH_FR2_VJ_Sequence_Frequencies.gnuplot")
            os.remove("./IGH_FR2_VJ_Sequence_Frequencies.tsv")
        ...
        for patient_id in lista_id:
            for folder in onlyfiles:
                if folder.startswith("index" + patient_id):
                    ...
                    loc_save="./CSV_files_IGH_FR2/" # location where to save the csv files
