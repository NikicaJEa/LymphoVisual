...
def IGH_FR1(folder_name, loc, loc_save, region, patient_id, region_graph, dic, svr, svc, sjr, sjc, srr, src,
    ...
    with open(loc_save + "index" + patient_id + ".fastq_read_summary_family.csv") as f:  
        ...
        mutation_rate = fastq_data.iloc[srr:srr + 10, src + 2]
    ...
    df['% total reads'] = list(total_reads_rounded)
    df['Mutation rate to partial V-gene %'] = list(mutation_rate)

    def IGH_FR1_table(data, col_width=10.0, row_height=0.625, font_size=14,
        ...
        fig.savefig("./plots/" + patient_id[0:3] + "_LymphoVisual/" + patient_id + "_IGH_FR1_table.png")
        return ax

    IGH_FR1_table(df, header_columns=0, col_width=5.0)
